...
Le flux : l'utilisateur choisit un ou plusieurs groupes de formation longue → l'app extrait les apprenants depuis Yparéo → les transforme au format d'import Gescicca → affiche un aperçu éditable → génère le CSV.
...
- `GET /api/r/v1/formation-longue/groupes?codesPeriode={selectedPeriode}` → alimente le **combobox de recherche multi-sélection des groupes** (input + liste filtrée `groupesFiltres`, filtrage insensible casse/accents via `sansAccents` / `libelleGroupe`, cases à cocher, la liste reste ouverte pendant la sélection, navigation clavier, fermeture au clic extérieur ; action « Sélectionner les N résultats » proposée seulement quand une recherche est saisie, pour éviter de tout cocher par mégarde). Rechargé (`useEffect([selectedPeriode])`, `groupesCharges`) à chaque changement de période. La source de vérité est `groupesSelectionnes` (tableau de codes, dans l'ordre de sélection) ; `handleChangerPeriode` le réinitialise avec `csvPreview`/`lignesSelectionnees`.
- **Une carte par groupe sélectionné** (sous le combobox, bouton × pour retirer, « Tout retirer » si plusieurs) contient : diplôme + plan de formation (cf. plus bas) et le champ **« Nom exact de la formation dans Gescicca »** propre au groupe (`nomsFormation`, objet indexé par code de groupe ; conservé si on retire puis rajoute le groupe).
...
- **Infos formation des groupes sélectionnés** (purement informatif, absentes du CSV) : à la sélection d'un groupe (`useEffect([groupesSelectionnes, groupes])`), affichage du diplôme (`abregeFormation`, code Gescicca) via `GET /api/r/v1/formations/{codeFormation}` (absent de `wrGroupe`, un appel dédié est nécessaire, une seule fois par `codeFormation` — cache `formationsInfo` + `formationsDemandeesRef`), et du **plan de formation du groupe** lié à l'année scolaire via `matieres` de chaque groupe (`wrMatiere[]`, triées par `matieresTriees`, déjà présent dans la réponse `formation-longue/groupes`, donc sans appel supplémentaire), affiché comme pastilles des `abregeMatiere` — celles marquées `nePlusUtiliser` restent affichées mais ressortent avec le code couleur des cellules fautives (`matiere-pastille-desactivee`) plutôt que d'être filtrées. Attention : `wrFormation.planDeFormation` (plan générique de la formation) est un piège — vérifié vide (`[]`) en test réel alors que `wrGroupe.matieres` contient les matières effectives du groupe ; c'est bien ce dernier qu'il faut utiliser.
...
- `GET /api/r/v1/groupes/{codeGroupe}/apprenants` → `wrApprenant[]` (chaque objet embarque déjà `inscriptions[]`, `informationsCourantes`, `adresse`). Un appel **par groupe sélectionné**, en parallèle (`Promise.allSettled`) : un groupe en échec (réponse non OK ou erreur réseau) est signalé (`groupesEnEchec`, encadré rouge sous « Extraire ») et les autres groupes s'affichent normalement.
- **Aperçu fusionné** : un seul tableau, lignes regroupées par groupe (ordre de sélection) ; `debutsGroupe` (indices) ajoute un trait plus épais au début de chaque groupe. `FORMATION` = nom exact saisi pour le groupe de la ligne, `GROUPE_FORMATION` = `nomGroupe`. Les doublons d'apprenant entre groupes sont volontairement ignorés (deux lignes).
...
- **Export Excel des erreurs** : bouton « Export Excel des erreurs » à côté de « Exporter en CSV », désactivé s'il n'y a aucune ligne fautive (`nbLignesFautives`). Exporte **toutes** les lignes fautives, indépendamment de la sélection, via `genererClasseurErreurs` (`src/exportErreursExcel.js`, testé dans `exportErreursExcel.test.js`) : cellules en erreur colorées (même rouge que `cellule-fautive`), colonne finale « Erreurs » (`COLONNE : message ; …`), toutes valeurs écrites en texte (préserve zéros initiaux, dates, téléphones). Fichier `erreurs_Gescicca_groupe_<groupe>.xlsx`. `exceljs` est chargé à la demande (`import()` dynamique, chunk séparé d'environ 255 kB gzip) pour ne pas alourdir le chargement initial.
- `handleExport` : ne sérialise que l'en-tête + les lignes cochées ; via `protegerSeparateur` (encadre chaque `;` interne d'une valeur par le caractère de protection Gescicca, cf. `SEPARATEUR_PROTECTION`), BOM `﻿` en tête, `Blob` téléchargé via un `<a download>` créé à la volée. Un seul fichier pour tous les groupes. Nom de fichier : `import_Gescicca_${libelleExport}.csv`, où `libelleExport` (mémorisé dans un state à l'extraction, plus d'index positionnel) vaut `groupe_<nom nettoyé par sanitizeNomFichier>` pour un groupe, ou `N_groupes` pour plusieurs (idem pour `erreurs_Gescicca_${libelleExport}.xlsx`).
...
Déjà traité : nom de fichier via index magique `csvPreview[1][23]` (→ `sanitizeNomFichier` + `libelleExport`), échappement du séparateur `;` (→ `protegerSeparateur`), race au chargement des référentiels (→ `referencesRef` + `referencesChargees`), validation des lignes avant export avec surlignage des cellules fautives (→ `src/gesciccaValidation.js`, cf. §3), sélection des lignes à exporter (cases à cocher, cf. §3), sélection de l'année scolaire (période Yparéo) pour extraire un groupe d'une année antérieure (cf. §1).
